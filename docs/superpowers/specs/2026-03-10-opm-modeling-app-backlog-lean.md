...
| ~~SIM-GAP-01~~ | ~~Medium~~ | ~~**Sin soporte para subprocesos paralelos (misma Y).** ISO §14.2.2.2 define orden parcial: subprocesos a la misma altura Y ejecutan en paralelo con barrier sync.~~ Wave-based execution con snapshot semantics implementado. | ~~§14.2.2.2~~ | **Fixeado** (7298355) |
| ~~SIM-GAP-02~~ | ~~Medium~~ | ~~**Falta invariante I-EVENT-INZOOM-BOUNDARY.** ISO §14.2.2.4.2: event links "shall not" cruzar contorno de proceso in-zoomed desde afuera hacia subprocesos.~~ Invariante añadido a validate(). Solo procesos (no objetos) cuentan como subprocesos. | ~~§14.2.2.4.2~~ | **Fixeado** (7298355) |
| ~~SIM-GAP-03~~ | ~~Medium~~ | ~~**Effect link encoding convention.** `source_state` en effect link es FROM state (precondición), `target_state` es TO state (postcondición). evaluatePrecondition ya no usa target_state como fallback para effect links.~~ | ~~§9.3.3.2, Annex C.16~~ | **Fixeado** (7298355) |
