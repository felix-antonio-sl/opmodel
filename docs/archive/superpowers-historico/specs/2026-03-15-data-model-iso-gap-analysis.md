...
|---|---|---|---|---|---|
| G-02 | Current state en Thing (no en State) | C.5, C.7 | Low | **Wrong location** | Refactor cross-cutting: migrar `State.current` → `Thing.current_state?: string`. Toca api.ts (3 funciones), 6+ tests, OpdCanvas, formatting textual legacy, serialization. Conocido como **O-01**. Requiere sesión TDD dedicada. |
